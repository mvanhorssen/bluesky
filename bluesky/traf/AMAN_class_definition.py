...
class AMAN(Flights):
    def __init__(self,AllFlights,unique_runways): #AllFlights contains information on all flights to be simulated
        ...
        self.Next_wpt_LAT=[] #LAT (nex waypoint)
        ...
        self.CurrEstTime_at_CBAS=[]
        self.CurrFlTime_to_CBAS=[]
        ...
        self.turnavoid_LON_used=[] #Waypoint LO for which the last possible (realistic) time has been established
        self.turnavoid_idx_used=[]
        ...
        self.LOG_time_at_CBAS=[]
        self.LOG_time_CBAS_passed=[]
        self.LOG_time_at_IAF=[] #Logger: time at IAF
        self.LOG_time_at_RWY=[] #Logger: time at RWY  
        self.LOG_accuracy_predepest_at_CBAS=[]
        ...
        self.initialize_values(unique_runways)        
    ...
    def initialize_values(self,unique_runways):
        ...
        for j in range(len(self.AllFlights.CallSign)):
            ...
            self.whichIAFs.append(self.AllFlights.Route_outside_TMA[j].whichIAF)            
            self.whichRWYs.append(self.AllFlights.Route_outside_TMA[j].whichRWY)
            ...
            self.CurrFlTime_to_nextwpt = np.append(self.CurrFlTime_to_nextwpt,0.)            
            ...
            self.CurrFlTime_to_IAF = np.append(self.CurrFlTime_to_IAF,self.AllFlights.Route_outside_TMA[j].estFT_to_IAF[0])
            self.CurrEstTime_at_RWY = np.append(self.CurrEstTime_at_RWY,self.AllFlights.PreDepEstTime_at_RWY[j]) #Estimated time at RWY
            self.CurrFlTime_to_RWY = np.append(self.CurrFlTime_to_RWY,self.AllFlights.Route_outside_TMA[j].estFT_to_RWY[0])   
            self.CurrEstTime_at_CBAS = np.append(self.CurrEstTime_at_CBAS,self.AllFlights.PreDepEstTime_at_CBAS[j])
            self.CurrFlTime_to_CBAS = np.append(self.CurrFlTime_to_CBAS,self.AllFlights.Route_outside_TMA[j].estFT_to_CBAS[0])			
            ...
            self.EnergyCost = np.append(self.EnergyCost,0.)     
            ...
            self.turnavoid_LAT_used = np.append(self.turnavoid_LAT_used,-999.)
            self.turnavoid_LON_used = np.append(self.turnavoid_LON_used,-999.)
            self.turnavoid_idx_used = np.append(self.turnavoid_idx_used,-1.)          
            
            self.LOG_time_at_CBAS = np.append(self.LOG_time_at_CBAS,-999.)
            self.LOG_time_CBAS_passed = np.append(self.LOG_time_CBAS_passed,-999.)
            self.LOG_CBAS_passed = np.append(self.LOG_CBAS_passed,False)
            ...
            self.LOG_accuracy_predepest_at_CBAS = np.append(self.LOG_accuracy_predepest_at_CBAS,-999.)
            ...
            self.LOG_energycost = np.append(self.LOG_energycost,-999.)        
    ...
    def update_SARA(self,BS_CallSign,BS_Altitude,BS_Route,SARA_horizon,simtime,approach_margin): #BS_altitude in m
        for k in range(len(BS_CallSign)):
            ...
            if self.SARA_sw[idx]=='ON' and self.CurrDelToBeAbs[idx]>approach_margin: #Aircraft needs to absorb positive delays
                ...
                # 2 situations: 1) Before almost IAF > speed reduction   2) Near almost IAF: decide on holding stack
                ##SITUATION 1: AIRCRAFT IS STILL BEFORE ALMOST IAF (speed changes possible)
                #Find active waypoint (outside TMA?)
                if (self.Next_wpt_name[idx] in self.AllFlights.Route_outside_TMA[idx].waypoints) and self.FinalDelAbsStrat_sw[idx]=='False':
                    wpt_index=self.AllFlights.Route_outside_TMA[idx].waypoints.index(self.Next_wpt_name[idx])
                    #Here it comes
                    self.MaxPossSpdDelAbs_to_nextwpt[idx]=self.MaxFT_to_nextwpt[idx]-self.CurrFlTime_to_nextwpt[idx] #minimum speed 
                    self.MaxPossSpdDelAbs_to_IAF[idx]=self.MaxFT_to_IAF[idx]-self.CurrFlTime_to_IAF[idx]
                
                    ratio=min(1.0,(self.CurrDelToBeAbs[idx]-approach_margin)/self.MaxPossSpdDelAbs_to_IAF[idx]) #ATTENTION! Approach margin of delay does not have to be absorbed!
                    ...
                    for z in range(wpt_index,len(self.AllFlights.Route_outside_TMA[idx].waypoints)):
                        ...
                        if z==wpt_index and self.MaxPossSpdDelAbs_to_nextwpt[idx]>=1.: #NECESSARY TO CHECK THAT DELAY THAT CAN BE ABSORBED IS POSITIVE. IT CAN BE SLIGHTLY NEGATIVE!
                ...
                BS_Route[k].calcfp() #BlueSky flight plan is recalculated too
                                
                ##SITUATION 2: AIRCRAFT IS NEARLY AT ALMOST IAF > DECIDE ON HOLDING PATTERN
                ...
                ##SITUATION 2.A: Delay smaller than 50/150 s > Dog leg (standard or 'dynamic' for RIVER)
                if S2A_1 or S2A_2:
                    #Define dog leg                    
                    ...
                    BS_Route[k].calcfp() #Recalculate flight plan (Top of descent etc.)                    
                ...
                ##SITUATION 2.B: Delay in between 50 and 150 s > Two semicircles
                    ...
                    speed_flown_TAS=self.AllFlights.Route_TMA[idx].spd_TAS[0] #Same speed as to IAF
                    speed_flown_CAS=self.AllFlights.Route_TMA[idx].spd[0] #Same speed as to IAF
                    time_flown_extra_semicircles=self.CurrDelToBeAbs[idx]#self.CurrDelToBeAbs[idx] #Variable delay between 50 and 150 s
                    ...
                    BS_Route[k].wpname.insert(tempindex_BS+5,'TWOSEMICIRCLES')
                ...
                ##SITUATION 2.C: Delay larger than 150 s > Holding stacks
            ...
            #Only do this loop if delay to be absorbed is larger (in absolute terms) than approach margin
                #1 situation: 1) Before almost IAF > speed reduction
                ...
                if (self.Next_wpt_name[idx] in self.AllFlights.Route_outside_TMA[idx].waypoints) and self.FinalDelAbsStrat_sw[idx]=='False':
                    wpt_index=self.AllFlights.Route_outside_TMA[idx].waypoints.index(self.Next_wpt_name[idx])
                    #Here it comes
                    self.MaxPossSpdAcc_to_nextwpt[idx]=self.CurrFlTime_to_nextwpt[idx]-self.MinFT_to_nextwpt[idx] #maximum speed 
                    self.MaxPossSpdAcc_to_IAF[idx]=self.CurrFlTime_to_IAF[idx]-self.MinFT_to_IAF[idx]
                    
                    ratio=min(1.0,(-self.CurrDelToBeAbs[idx]-approach_margin)/self.MaxPossSpdAcc_to_IAF[idx]) #ATTENTION! Approach margin of delay does not have to be absorbed!
                    ratio=max(0.,ratio)                    
                    ##LIMIT RATIO TO 1.! You cannot increase more speed than what is possible!                           
                    
                    for z in range(wpt_index,len(self.AllFlights.Route_outside_TMA[idx].waypoints)):
                        #First up to the next waypoint
                        if z==wpt_index and self.MaxPossSpdAcc_to_nextwpt[idx]>=1.: #NECESSARY TO CHECK THAT DELAY THAT CAN BE ABSORBED IS POSITIVE. IT CAN BE SLIGHTLY NEGATIVE!
    ...
    def scheduler_ASAP_basic(self,BS_CallSign,AMAN_horizon,RWY,intarrtime_RWY,STA_Freeze_horizon,unique_runways): 
        ...
        if newac_counter==0: # No new aircraft within horizon
        ...
        elif newac_counter>0: # New aircraft within horizon
            for z in range(newac_counter):
                ...
                if ETA_newac>last_semifixed_ETA and self.CurrDirectDist_to_APT[idx_newac]>STA_Freeze_horizon:                 
                    ...
                    self.scheduler_ASAP_subfunction_update_status(BS_CallSign,RWY,STA_Freeze_horizon)
                    last_fixed_STA=self.scheduler_ASAP_subfunction_find_last_fixed_STA(BS_CallSign,RWY)
    ...
    def calculate_energy_cost_subfunction(self,RC,TAS,CD_0,CD_2,rho,S,phi,mass_nominal,delta_t): #SI UNITS!!! #phi=bank-angle #delta_t=time-step
    ...
    def continuous_turn_avoider(self,BS_CallSign,BS_route,BS_ActWptLAT,BS_ActWptLON,BS_traf_object,simtime):
        for k in range(len(BS_CallSign)): #Callsign is array from BlueSky traffic, implies that aircraft is airborne
            ...
            if self.CurrDirectDist_to_APT[idx]>40. or self.CurrLoc[idx]=='TMA': #Only do this when still outside TMA, otherwise there might be problems (infinite holding)
                ...
                #Options: 1) last possible time not established yet, but it is time to do so
    ...
    def scheduler_ASAP_upgrade(self,BS_CallSign,AMAN_horizon,RWY,intarrtime_RWY,STA_Freeze_horizon,unique_runways): 
        ...
        if normal_newac_counter==0 and popup_newac_counter==0:
            self.scheduler_ASAP_subfunction_update_status(BS_CallSign,RWY,STA_Freeze_horizon) #Re-use function
            last_fixed_STA=self.scheduler_ASAP_subfunction_find_last_fixed_STA(BS_CallSign,RWY) #Re-use function
            last_semifixed_ETA,last_semifixed_STA=self.scheduler_ASAP_upgrade_subfunction_find_last_semifixed_ETAs_STAs(BS_CallSign,RWY,last_fixed_STA)
            self.scheduler_ASAP_upgrade_subfunction_update_varschedule(BS_CallSign,RWY,intarrtime_RWY,last_semifixed_STA) #Re-use function
            
        if normal_newac_counter>0: #First update the NORMAL new aircraft
            for z in range(normal_newac_counter):
                ...
                if ETA_newac_normal>last_semifixed_ETA and self.CurrDirectDist_to_APT[idx_newac_normal]>STA_Freeze_horizon:                 
                    ...
                    self.scheduler_ASAP_upgrade_subfunction_update_varschedule(BS_CallSign,RWY,intarrtime_RWY,last_semifixed_STA)
                ...
                #Situation B.2
                    ...
                    last_semifixed_STA,last_semifixed_ETA=self.scheduler_ASAP_upgrade_subfunction_update_semifixed_using_ETAs(BS_CallSign,RWY,intarrtime_RWY,ETA_newac_normal,idx_newac_normal,last_fixed_STA)
                    last_semifixed_ETA,last_semifixed_STA=self.scheduler_ASAP_upgrade_subfunction_find_last_semifixed_ETAs_STAs(BS_CallSign,RWY,last_fixed_STA)               
                    self.scheduler_ASAP_upgrade_subfunction_update_varschedule(BS_CallSign,RWY,intarrtime_RWY,last_semifixed_STA)
        ...
        if popup_newac_counter>0: #Then update the POPUP new aircraft
            for z in range(popup_newac_counter):
                ...
                #Two situations possible: 1) actual takeoff time>planned and 2) actual takeoff<=planned
                ...
                if swC1 or swC2:
                    ...
                    last_semifixed_STA,last_semifixed_ETA=self.scheduler_ASAP_upgrade_subfunction_update_semifixed_using_ETAs(BS_CallSign,RWY,intarrtime_RWY,ETA_newac_popup,idx_newac_popup,last_fixed_STA)
                    last_semifixed_ETA,last_semifixed_STA=self.scheduler_ASAP_upgrade_subfunction_find_last_semifixed_ETAs_STAs(BS_CallSign,RWY,last_fixed_STA)                    
                    self.scheduler_ASAP_upgrade_subfunction_update_varschedule(BS_CallSign,RWY,intarrtime_RWY,last_semifixed_STA)
                    ...
                    if callsign_right_after_gap!=[]:
                        ...
                        last_semifixed_STA,last_semifixed_ETA=self.scheduler_ASAP_upgrade_subfunction_update_semifixed_using_ETAs(BS_CallSign,RWY,intarrtime_RWY,earliest_ETA,temptempidx,last_fixed_STA)
                        last_semifixed_ETA,last_semifixed_STA=self.scheduler_ASAP_upgrade_subfunction_find_last_semifixed_ETAs_STAs(BS_CallSign,RWY,last_fixed_STA)                    
                        self.scheduler_ASAP_upgrade_subfunction_update_varschedule(BS_CallSign,RWY,intarrtime_RWY,last_semifixed_STA)
